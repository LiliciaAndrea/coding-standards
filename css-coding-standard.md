...
#### [强制] 当一个 rule 包含一个 selector 时，只占一行。
...
#### [强制] Class命名为全小写，短而明确但要避免过度使用简写

解释：
可以使用短划线（不要使用下划线和 camelCase 命名）。短划线应该作为相关类的自然间断。(例如，.btn 和 .btn-danger)。使用有意义的名称；使用结构化或者作用目标相关，而不是抽象的名称。

#### [强制] 命名时使用最近的父节点或者父 class 作为前缀。

#### [强制] 使用`.js-* classes`来表示行为(相对于样式)，但是不要在 CSS 中包含这些`classes`。

#### [强制] 使用 classes 而不是通用元素标签来优化渲染性能。

#### [强制] 避免在经常出现的组件中使用一些属性选择器 (例如，[class^="..."])。浏览器性能会受到这些情况的影响。

#### [强制] 减少选择器的长度，每个组合选择器选择器的条目应该尽量控制在 3 个以内。

#### [强制] 只在必要的情况下使用后代选择器 (例如，没有使用带前缀 classes 的情况).

Read This: 
- [Scope CSS classes with prefixes](http://markdotto.com/2012/02/16/scope-css-classes-with-prefixes/)
- [Stop the cascade](http://markdotto.com/2012/02/16/scope-css-classes-with-prefixes/)
...
- 多用几个`<link>`标签
- 将你的css编译到一个文件里
- 用Rails, Jekyll,以及其他环境提供的特性来间接这些css

更多信息, 阅读 [dont-use-import](http://www.stevesouders.com/blog/2009/04/09/dont-use-import/).
...
带有alpha的颜色信息可以使用 `rgba()`。使用 `rgba()` 时每个逗号后必须保留一个空格。并且不要带有取值前面不必要的 0 (比如，使用 .5 替代 0.5)。
...
在 Textmate 中，使用 Text → Edit Each Line in Selection。 
...
> Written by [leaf-hsiao][1] @ [千年弦歌][2]


  [1]: http://blog.leafh.pw/
  [2]: http://1000.hnu.cn